...
IPV6ADDR=2001:db8:a::10/64
IPV6_DEFAULTGW=2001:db8:a::1
IPV6_DEFAULTDEV=em1
"""
...
_ROUTES_V6 = """default via 2001:db8:a::1 dev em1
...
_OVS_INTERFACE_WITH_EXTRA = _OVS_INTERFACE + """OVS_EXTRA=\"{ovs_extra}\"
"""
...
class TestIfcfgNetConfig(base.TestCase):
    ...
    def test_network_with_ipv6_routes(self):
        ...
        route3 = objects.Route('192.168.1.5', '172.20.0.0/24',
                               route_options="metric 100")
        route4 = objects.Route('2001:db8:a::1', default=True)
        ...
        route6 = objects.Route('fd00:fd00:2000::1',
                               ...
                               route_options="metric 100")
        v4_addr = objects.Address('192.168.1.2/24')
        v6_addr = objects.Address('2001:db8:a::10/64')
    ...
    def test_network_ovs_bridge_with_ovs_extra(self):
        ovs_extra = ["set interface em1 external-ids:foo=bar"]
        interface = objects.Interface('em1', ovs_extra=ovs_extra)
        bridge = objects.OvsBridge('br-ctlplane', use_dhcp=True,
                                   members=[interface])
        self.provider.add_bridge(bridge)
        self.provider.add_interface(interface)
        self.assertEqual(
            _OVS_INTERFACE_WITH_EXTRA.format(ovs_extra='--'.join(ovs_extra)),
            self.get_interface_config()
        )
        self.assertEqual(_OVS_BRIDGE_DHCP,
                         self.provider.bridge_data['br-ctlplane'])

    def test_nic_part_ovs_bridge_with_ovs_extra(self):
        ...
        ovs_extra = ["set interface eth2_7 external-ids:foo=bar"]
        vf = objects.SriovVF(
            device='eth2',
            vfid=7,
            ovs_extra=ovs_extra
...
"""
    ...
    def test_ovs_user_bridge_no_ovs_internal(self):
        bridge = objects.OvsUserBridge(name='br-no-int', ovs_internal=False)
        self.provider.add_ovs_user_bridge(bridge)
        user_br_no_ovs_int = """# This file is autogenerated by os-net-config
DEVICE=br-no-int
ONBOOT=yes
HOTPLUG=no
NM_CONTROLLED=no
...
LINKSTATUS=down
"""
        self.assertEqual(user_br_no_ovs_int,
                         self.provider.bridge_data['br-no-int'])
...
class TestIfcfgNetConfigApply(base.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_sysctl_file = tempfile.NamedTemporaryFile()
